...
function getTip(tx: Transaction): string {
  if (tx.type === 'income') {
    return `Dépôt reçu. Pense à allouer 20% (${(tx.amount * 0.2).toFixed(0)} $) à ton épargne.`;
  }
  if (tx.categoryName === 'Restaurants') {
    return `À ${tx.amount.toFixed(2)} $ par visite, réduire la fréquence peut libérer du budget mensuel.`;
  }
  return 'Transaction enregistrée. Vérifie la catégorie pour un meilleur suivi.';
}
...
function getSyncStatusMeta(status: Transaction['syncStatus'], colors: AppColors): { icon: IconName; label: string; color: string } {
  if (status === 'synced') {
    return { icon: 'cloud-done-outline', label: 'Synchronisé', color: colors.success };
  }
  if (status === 'failed') {
    return { icon: 'cloud-offline-outline', label: 'Échec de synchronisation', color: colors.danger };
  }
        ...
        <SurfaceCard style={styles.receiptCardShell} innerStyle={styles.receiptCardInner} padding={spacing.md}>
          ...
          {tx.receiptUri && !tx.receiptUri.startsWith('scan://') ? (
            ...
            <Text style={styles.receiptMeta}>
              {tx.receiptStatus === 'scan_pending' ? 'Reçu marqué pour scan.' : 'Reçu disponible.'}
            </Text>
          )}
        </SurfaceCard>
      ) : null}
